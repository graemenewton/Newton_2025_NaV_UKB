...
model <- glm(chronic_pain ~ group_rare_mutations + age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = phenos, family = binomial())
...
#
```{r}
mut_count <- data.frame(table(phenos$mutation))
```
...
## Topology Plot
```{r}
topol_df <- table(mut$topology[which(mut$UKB_het < 50)])
```
